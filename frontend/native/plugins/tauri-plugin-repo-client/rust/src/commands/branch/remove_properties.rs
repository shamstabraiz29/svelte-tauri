...
use common_dtos::commit_client::{
    repo_modify::{
        response::RepoModifyResponse, BranchModifyRequestBuilder, BranchRemovePropertiesRequest,
    },
    AgOperationStatus,
};
...
use crate::{
    ...
    models::remove_branch_properties::{
        request::RemoveBranchPropertiesRequest, response::RemoveBranchPropertiesResponse,
    },
};

#[tauri::command]
#[specta::specta]
pub async fn remove_branch_properties(
    ...
    request: RemoveBranchPropertiesRequest,
) -> Result<RemoveBranchPropertiesResponse> {
    ...
    let commit_fn = Box::new(
        move |acct_api_client: RepoApiClient,
            ...
            Box::pin(async move {
                let payload =
                    BranchModifyRequestBuilder::<BranchRemovePropertiesRequest>::default()
                        .set_repo_id(repo_id)
                ...
                let RepoModifyResponse::BranchRemoveProperties(response) = response else {
                    return Err(Error {
    ...
    Ok(RemoveBranchPropertiesResponse { req_id })
}
