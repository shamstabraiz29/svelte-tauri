...
use common_dtos::commit_client::{
    repo_modify::{response::RepoModifyResponse, BranchModifyRequestBuilder, BranchRemoveRequest},
    AgOperationStatus,
};
...
use crate::{
    ...
    models::remove_branch::{request::RemoveBranchRequest, response::RemoveBranchResponse},
};
...
pub async fn remove_branch(
    ...
    let commit_fn = Box::new(
        move |acct_api_client: RepoApiClient,
            ...
            Box::pin(async move {
                let payload = BranchModifyRequestBuilder::<BranchRemoveRequest>::default()
                    .set_repo_id(repo_id)
                ...
                let RepoModifyResponse::BranchRemove(response) = response else {
                    return Err(Error {
    ...
    Ok(RemoveBranchResponse { req_id })
}
