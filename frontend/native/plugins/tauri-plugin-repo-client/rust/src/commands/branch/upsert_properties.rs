...
use common_dtos::commit_client::{
    repo_modify::{
        response::RepoModifyResponse, BranchModifyRequestBuilder, BranchUpsertPropertiesRequest,
    },
    AgOperationStatus,
};
...
use crate::{
    ...
    models::upsert_branch_properties::{
        request::UpsertBranchPropertiesRequest, response::UpsertBranchPropertiesResponse,
    },
};

#[tauri::command]
#[specta::specta]
pub async fn upsert_branch_properties(
    ...
    request: UpsertBranchPropertiesRequest,
) -> Result<UpsertBranchPropertiesResponse> {
    ...
    let commit_fn = Box::new(
        move |acct_api_client: RepoApiClient,
            ...
            Box::pin(async move {
                let payload =
                    BranchModifyRequestBuilder::<BranchUpsertPropertiesRequest>::default()
                        .set_repo_id(repo_id)
                ...
                let RepoModifyResponse::BranchUpsertProperties(response) = response else {
                    return Err(Error {
    ...
    Ok(UpsertBranchPropertiesResponse { req_id })
}
