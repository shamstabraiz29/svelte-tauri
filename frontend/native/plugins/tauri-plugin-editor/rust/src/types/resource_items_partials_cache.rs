...
use common_dtos::editor_client::resource_items_partials_get::{
    request::ResourceItemsPartialsGetRequest, response::ResourceItemsPartialsGetStatus,
};
...
impl ResourceItemsPartialsCache {
    ...
    pub(crate) async fn get_resource_items_partials<R: Runtime>(
        &self,
        app: &AppHandle<R>,
...

        {
            let loaded_data = self
                .loaded_data
                .lock()
                .map_err(|_| InternalApplicationError::ResourceItemsPartialsLock)?;
        ...
        let payload = ResourceItemsPartialsGetRequest {
            per_viewport_resource_items_types: non_loaded_static_data.to_owned(),
        };
        let node_info_get_response = editor_api_client
            .get_resource_items_partials(&access_token, payload)
            .await?;

        match node_info_get_response.status {
            ResourceItemsPartialsGetStatus::Success {
                resource_items_partials: viewport_resource_item_type_static_data,
            } => {
                {
                        ...
                        viewport_resource_item_type_static_data
                    {
                        ...
                        let requested_partials = requested_static_data
                            .entry(viewport_type.to_owned())
                            .or_default();
            ...
            ResourceItemsPartialsGetStatus::Failure(msg) => {
