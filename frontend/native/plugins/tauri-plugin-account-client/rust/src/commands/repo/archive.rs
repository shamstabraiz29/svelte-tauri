...
use common_dtos::commit_client::{
    acct_modify::{response::AcctModifyResponse, RepoArchiveRequest, RepoModifyRequestBuilder},
    AgOperationStatus,
};
...
use crate::{
    ...
    models::archive_repo::{request::ArchiveRepoRequest, response::ArchiveRepoResponse},
};
...
pub async fn archive_repo(
    ...
    request: ArchiveRepoRequest,
) -> Result<ArchiveRepoResponse> {
    ...
    let commit_fn = Box::new(
        move |acct_api_client: AccountApiClient,
            ...
            Box::pin(async move {
                let payload = RepoModifyRequestBuilder::<RepoArchiveRequest>::default()
                    .set_acct_id(acct_id)
                ...
                let AcctModifyResponse::RepoArchive(response) = response else {
                    return Err(Error {
