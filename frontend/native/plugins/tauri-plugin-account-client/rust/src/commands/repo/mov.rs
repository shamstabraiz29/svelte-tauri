...
use common_dtos::commit_client::{
    acct_modify::{response::AcctModifyResponse, RepoModifyRequestBuilder, RepoMoveRequest},
    AgOperationStatus,
};
...
use crate::{
    commands::{
        assert_folders_exists::assert_folders_exits, assert_repos_exists::assert_repos_exits,
        execute_modify_action::execute_modify_action,
    },
    config::AccountClientConfig,
    lifecycle::init::AcctCommitInfo,
    models::move_repo::{request::MoveRepoRequest, response::MoveRepoResponse},
};
...
pub async fn move_repo(
...
) -> Result<MoveRepoResponse> {
    let req_id = request.req_id.clone();
    let repo_id = request.repo_id.clone();

    assert_folders_exits(&acct_ag_state, &[&request.parent_folder_id]).map_err(|msg| Error {
        req_id: req_id.clone(),
        message: msg,
    })?;
    ...
    let commit_fn = Box::new(
        move |acct_api_client: AccountApiClient,
            ...
            Box::pin(async move {
                let payload = RepoModifyRequestBuilder::<RepoMoveRequest>::default()
                ...
                let AcctModifyResponse::RepoMove(response) = response else {
                    return Err(Error {
                        req_id,
