...
use common_dtos::commit_client::{
    acct_modify::{
        response::AcctModifyResponse, FolderModifyRequestBuilder, FolderUpsertPropertiesRequest,
    },
    AgOperationStatus,
};
...
use crate::{
    ...
    models::upsert_folder_properties::{
        request::UpsertFolderPropertiesRequest, response::UpsertFolderPropertiesResponse,
    },
};

#[tauri::command]
#[specta::specta]
pub async fn upsert_folder_properties(
    ...
    request: UpsertFolderPropertiesRequest,
) -> Result<UpsertFolderPropertiesResponse> {
    ...
    let commit_fn = Box::new(
        move |acct_api_client: AccountApiClient,
            ...
            Box::pin(async move {
                let payload =
                    FolderModifyRequestBuilder::<FolderUpsertPropertiesRequest>::default()
                        .set_acct_id(acct_id)
                ...
                let AcctModifyResponse::FolderUpsertProperties(response) = response else {
                    return Err(Error {
    ...
    Ok(UpsertFolderPropertiesResponse { req_id })
}
