...
use crate::{
    ...
    models::remove_folder_properties::{
        request::RemoveFolderPropertiesRequest, response::RemoveFolderPropertiesResponse,
    },
};
use common_dtos::commit_client::{
    acct_modify::{
        response::AcctModifyResponse, FolderModifyRequestBuilder, FolderRemovePropertiesRequest,
    },
    AgOperationStatus,
};
...
#[tauri::command]
#[specta::specta]
pub async fn remove_folder_properties(
    ...
    request: RemoveFolderPropertiesRequest,
) -> Result<RemoveFolderPropertiesResponse> {
    ...
    let commit_fn = Box::new(
        move |acct_api_client: AccountApiClient,
            ...
            Box::pin(async move {
                let payload =
                    FolderModifyRequestBuilder::<FolderRemovePropertiesRequest>::default()
                        .set_acct_id(acct_id)
                ...
                let AcctModifyResponse::FolderRemoveProperties(response) = response else {
                    return Err(Error {
    ...
    Ok(RemoveFolderPropertiesResponse { req_id })
}
